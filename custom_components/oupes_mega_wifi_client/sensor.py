"""Sensor entities for the OUPES Mega WiFi Client integration."""
...
def _device_info(coordinator: OUPESWiFiClientCoordinator) -> DeviceInfo:
    from .const import model_name_from_product_id
    return DeviceInfo(
...
class OUPESWiFiSensor(
    CoordinatorEntity[OUPESWiFiClientCoordinator], SensorEntity
):
    ...
    def native_value(self) -> float | str | None:
        ...
        if desc.slot is not None:
            raw = self.coordinator.data.get("ext_batteries", {}).get(
                desc.slot, {}
            ).get(lookup)
        else:
...
async def async_setup_entry(
    ...
    async_add_entities(
        OUPESWiFiSensor(coordinator, _resolve(desc), entry)
    ...
    def _add_new_slots() -> None:
        ...
        if not new_slots:
            return
        new_entities = [
            OUPESWiFiSensor(coordinator, desc, entry)
