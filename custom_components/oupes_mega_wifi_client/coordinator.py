...
class OUPESWiFiClientCoordinator(DataUpdateCoordinator[OUPESData]):
    ...
    def start(self) -> None:
        ...
        if self._connection_task is None or self._connection_task.done():
            ...
            self._connection_task = self.hass.async_create_background_task(
                ...
                name=f"oupes_wifi_client_{self.device_id}",
            )
    ...
    async def _run_connection_loop(self) -> None:
        ...
        while not self._stopping:
            try:
                ...
                _LOGGER.warning(
                    "WiFi client %s connection error: %s — reconnecting in %ds",
    ...
    async def _connect_and_run(self) -> None:
        ...
        _LOGGER.info(
            "WiFi client: connecting to %s:%d for device %s",
        ...
        try:
            ...
            self._close_writer()
            _LOGGER.info(
                "WiFi client: disconnected from %s:%d (device %s)",
                self.host, self.tcp_port, self.device_id,
            )
    ...
    async def _run_session(self) -> None:
        ...
        The proxy server handles periodic polling; this client only
    ...
    def _handle_line(self, line: str) -> None:
        ...
        if cmd == "publish":
            ...
            _LOGGER.debug("WiFi client %s: subscribe ACK", self.device_id)

        elif cmd == "keep":
            _LOGGER.debug("WiFi client %s: keep ACK", self.device_id)
    ...
    def _send_line(self, line: str) -> None:
        if self._writer and not self._writer.is_closing():
            try:
                ...
                _LOGGER.debug(
                    "WiFi client %s: send error: %s", self.device_id, exc
                )
