...
1. Copy the `custom_components/oupes_mega_ble/` folder into your HA configuration
   directory so the path becomes:

   ```
   <ha-config>/custom_components/oupes_mega_ble/
   ```
...
logger:
  logs:
    custom_components.oupes_mega_ble: debug
```
